...
void binomialTheorique(int n, double p, int k)
{
    ...
    q        = 1 - p;
    binomial = static_cast<double>(fac(n)) / (fac(k) * fac(n - k)) * pow(p, k) * pow(q, n - k);
...
{
    scoreJoueurTxt     = std::u16string(1, static_cast<char16_t>(scoreUtilisateur + u'0'));
    scoreOrdinateurTxt = std::u16string(1, static_cast<char16_t>(scoreOrdinateur + u'0'));
}
